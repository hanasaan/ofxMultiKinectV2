...
void Freenect2DeviceImpl::stop()
{
  ...
  UsbControl::ResultCode ret = usb_control_.setIrInterfaceState(UsbControl::Disabled);
    if (ret == UsbControl::Error) {
        state_ = Open;
        return;
    }
  ...
  ret = usb_control_.setVideoTransferFunctionState(UsbControl::Disabled);
    if (ret == UsbControl::Error) {
        state_ = Open;
        return;
    }
