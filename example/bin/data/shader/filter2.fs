...
uniform sampler2DRect MaxEdgeTest;

uniform Parameters Params;

void applyEdgeAwareFilter(ivec2 uv)
{
  vec2 v = texelFetch2DRect(DepthAndIrSum, uv).xy;
  ...
  if(v.x >= Params.min_depth && v.x <= Params.max_depth)
  {
    ...
    {
      bool max_edge_test_ok = texelFetch2DRect(MaxEdgeTest, uv).x > 0.1;
      ...
      for(int yi = -1; yi < 2; ++yi)
      {
        for(int xi = -1; xi < 2; ++xi)
        {
          ...
          vec2 other = texelFetch2DRect(DepthAndIrSum, uv + ivec2(xi, yi)).xy;
  ...
  else
  {
    ...
    gl_FragData[0] = vec4(vec3(clamp(FilterDepth/65536.0, 0.0, 1.0)), 1.0);
}
